...
/**
 * A selector. As there is a problem with `swagger-codegen` generating OAS3.x `oneOf` we are using an internal encoding for now to support Patterns for `desc` and `text`. We use an special string that starts with a literal `Pattern:` followed by the actual pattern. For example `Pattern:^[a-z]+$` * @param checkable  * @param clazz  * @param clickable  * @param depth  * @param desc  * @param pkg  * @param res  * @param scrollable  * @param text  * @param index  * @param instance */
