...
/**
 * A selector. As there is a problem with `swagger-codegen` generating OAS3.x `oneOf` we are using an internal encoding for now to support Patterns for `desc` and `text`. We use an special string that starts with a literal `Pattern:` followed by the actual pattern. For example `Pattern:^[a-z]+$`
 * @param checkable 
 * @param checked 
 * @param clazz 
 * @param clickable 
...
data class Selector (
    ...
    val hasChild: Selector? = null,
    val hasDescendant: Selector? = null,
    val pkg: kotlin.String? = null,
    val res: kotlin.String? = null,
